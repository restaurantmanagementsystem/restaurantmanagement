...
//import db details over here


class Cart {
    static addToCart(orderitem = null, qty = 1, cart) {
        if(!this.inCart(orderitem.orderItemId, cart)) {
           // let format = new Intl.NumberFormat(config.locale.lang, {style: 'currency', currency: config.locale.currency });
            let prod = {
              id: orderitem.orderItemId,
              title: orderitem.orderItemId,
              price: orderitem.orderItemId,
              qty: qty,
              //image: product.image,
              formattedPrice: format.format(orderitem.price)
            };
